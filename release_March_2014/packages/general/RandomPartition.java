...
public class RandomPartition {
	...
	// random partition of numbers (0, 1, ..., n-1)
	public int[] get() {
		...
		//Convert the partition to a coalition structure, where every coalition 
		//is a number in which true bits represent the members
		int[] coalitionStructure = new int[ coalitionsCount ];
		for(int i=0; i<coalitionsCount; i++){
			coalitionStructure[i]=0;
		}
		for(int i=0; i<n; i++){
			coalitionStructure[ partition[i]-1 ] += (1<<i); //add agent "i+1" to the coalition
		}
